...
window.SIM = {
  ...
  /* 받을 토큰은 받을 네트워크가 정해져야 고를 수 있다. 순서를 어기면 이 말이 뜬다. */
  pickOrder: '받을 네트워크부터 정해야 해. 어떤 토큰이 있는지는 네트워크마다 다르거든.',

  /* 아직 차례가 아닌 것을 눌렀을 때. 눌리지도 설명도 없으면 고장으로 읽힌다.
     {WHAT} 에는 지금 열어야 할 라벨 이름이 들어간다. {WHAT}{을} 은 받침에 맞춰 을/를 로 바뀐다. */
  openOrder: '{WHAT}부터 열어 봐. 차례대로 열어야 무엇이 무엇인지 보이거든.',

  /* 아직 못 누르는 버튼을 눌렀을 때. 무엇이 남았는지 말해 준다. */
  waitWhy: {
    veil: '아직 가려진 것이 있어. {WHAT}{을} 눌러서 열어 봐.',
    mission: '무엇을 해 볼지 먼저 골라야 해.',
    pick: '받을 네트워크와 토큰을 먼저 골라야 해.'
  },
