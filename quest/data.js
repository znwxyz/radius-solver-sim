...
window.MODE = {
  ...
  board: {
    ...
    empty: '아직 안 해 봤어',
    doing: '진행 중',
    note: '서명 횟수는 지갑이 너에게 서명을 물어본 횟수야. 가스는 네 지갑에서 실제로 나간 돈만 세.'
  },

  /* ── 비교 화면 ── */
  compare: {
    title: '달라진 건 결과가 아니라 과정이야',
    sub: '일반 모드에서도 솔버 모드에서도 Robinhood Chain에 USDC가 도착했어. 달랐던 건 네가 직접 해야 했던 일의 양이야.',
    pickLine: '일반 모드에서 너는 브릿지로 {WHO}를 골랐어.',
    /* 두 모드가 다 끝나면 숫자를 보고 한 줄을 고른다. 공식 브릿지(수수료 0)를 고르면
       USDC 는 직접 한 쪽이 더 받으므로 mixed 가 뜬다. */
    verdict: {
      cheaper: '솔버 모드에서 너는 ETH를 {DIFF} 덜 냈고, USDC는 {GOT} 더 받았어.',
      mixed: 'USDC는 네가 직접 한 쪽이 {GOT} 더 받았지만, ETH는 솔버 모드에서 {DIFF} 덜 냈고, 직접 한 쪽은 {WAIT}을 기다려야 했어.'
    },
    notes: [
      '솔버는 프로토콜의 규칙 안에서 거래를 대신 실행하는 독립 참여자야. Radius도 그런 솔버 중 하나야.',
      '모든 스왑에 솔버가 붙는 건 아니야. 어떤 경로로 거래할지는 네가 쓰는 앱과 프로토콜이 정해.'
    ],
    cta: '처음부터 다시 해 볼까?'
  },
